...
try:
    ...
    def allocate_buffers(engine):
        h_input = cuda.pagelocked_empty(
            trt.volume(engine.get_binding_shape(0)), dtype=trt.nptype(DTYPE)
        )
        h_output = cuda.pagelocked_empty(
            trt.volume(engine.get_binding_shape(1)), dtype=trt.nptype(DTYPE)
        )
        ...
        if iterations is None:
            ...
            while elapsed_time < 1:
                ...
                for _ in range(iterations):
                    context.execute(
                        batch_size=1, bindings=[int(d_input), int(d_output)]
                    )
        ...
        with build_engine("model.onnx") as engine:
            ...
            with engine.create_execution_context() as context:
                latency = do_inference(
                    context, h_input, d_input, h_output, d_output, iterations=iterations
                )
    ...
    warnings.warn(
        "TensorRT (or pycuda) is not installed. compute_latency_ms_tensorrt() cannot be used."
    )
#########################################################################
...
def plot_path(lasts, paths=[]):
    ...
    if path0[-1] != lasts[0]:
        path0.append(lasts[0])
    if len(path1) != 0 and path1[-1] != lasts[1]:
        path1.append(lasts[1])
    if len(path2) != 0 and path2[-1] != lasts[2]:
        path2.append(lasts[2])
    ...
    ax.plot(
        np.arange(len(path2)),
        2 - np.array(path2) - 0.16,
        lw=1.2,
        label="1/8",
        color="#5a5858",
        linestyle="-.",
    )  # , marker='s', markeredgecolor='m', markerfacecolor='m')
...
def plot_path_width(lasts, paths=[], widths=[]):
    ...
    assert (
        len(path0) == len(width0) + 1 or len(path0) + len(width0) == 0
    ), "path0 %d, width0 %d" % (len(path0), len(width0))
    assert (
        len(path1) == len(width1) + 1 or len(path1) + len(width1) == 0
    ), "path1 %d, width1 %d" % (len(path1), len(width1))
    assert (
        len(path2) == len(width2) + 1 or len(path2) + len(width2) == 0
    ), "path2 %d, width2 %d" % (len(path2), len(width2))
    ...
    ax.plot(
        np.arange(len(path2)),
        2 - np.array(path2) + line_updown * 2,
        lw=1.2,
        label="1/8",
        color="#5a5858",
        linestyle="-.",
    ...
    for idx, width in enumerate(width2):
        annotations[(idx, path2[idx], width, path2[idx + 1] - path2[idx])] = (
            (
                0.35 + idx,
                2
                - path2[idx]
    ...
    for idx, width in enumerate(width1):
        annotations[(idx, path1[idx], width, path1[idx + 1] - path1[idx])] = (
            (
                0.35 + idx,
                2
                - path1[idx]
                + line_updown
                + annotation_updown
                - (path1[idx + 1] - path1[idx]) / annotation_down_scale,
            ),
            width,
        )
    for idx, width in enumerate(width0):
        annotations[(idx, path0[idx], width, path0[idx + 1] - path0[idx])] = (
            (
                0.35 + idx,
                2
                - path0[idx]
                + annotation_updown
                - (path0[idx + 1] - path0[idx]) / annotation_down_scale,
            ),
            width,
        )
