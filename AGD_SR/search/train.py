...
def main():
    ...
    train_loader_model = DataLoader(
        ImageDataset(
            config.dataset_path_train, transforms_=transforms_, unaligned=True
        ),
...
def train(
    train_loader_model, model, teacher_model, optimizer, lr_policy, logger, epoch
):
    model.train()
...
def infer(epoch, model, test_loader, logger):
    ...
    psnr = compute_psnr("output/B_nasgan", config.dataset_path_val_hr)
