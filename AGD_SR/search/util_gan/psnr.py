...
def compute_psnr(folder_Gen, folder_GT, test_Y=True):
    # Configurations
    folder_Gen = pathlib.Path(folder_Gen)
    folder_GT = pathlib.Path(folder_GT)
    ...
    img_list_gt = sorted(folder_GT.glob("*"), key=lambda e: e.name)

    # img_list_gen = sorted(glob.glob(folder_Gen + '/*'))
    img_list_gen = sorted(folder_Gen.glob("*"), key=lambda e: e.name)
    ...
    for i, img_path in enumerate(img_list_gt):
        im_GT = cv2.imread(str(img_path)) / 255.0
        # im_Gen = cv2.imread(os.path.join(folder_Gen, base_name + suffix + '.png')) / 255.
        im_Gen = cv2.imread(str(img_list_gen[i])) / 255.0
