...
class UniformQuantize(InplaceFunction):
    ...
    def forward(
        ...
        if qparams is None:
            ...
            ), "either provide qparams of num_bits to quantize"
...
def quantize_grad(
    ...
    return UniformQuantizeGrad().apply(
...
class QuantMeasure(nn.Module):
    ...
    def forward(self, input, qparams=None):
        ...
        if self.training or self.measure:
            ...
            qparams = QParams(
                range=self.running_range,
                zero_point=self.running_zero_point,
                num_bits=self.num_bits,
            )
        if self.measure:
            return input
