...
class Architect(object):
    def __init__(self, model, args, distill=False):
        ...
        self.optimizer = torch.optim.Adam(
            list(self.model.module._arch_params.values()),
    ...
    def _backward_step(self, input_valid, target_valid):
        ...
        if len(self.model.module._width_mult_list) == 1:
            ...
            flops = flops + r0 * self.model.module.forward_flops(
                (3, 510, 350), alpha=True, beta=False, ratio=False
            )

            if self.quantize == "search":
                flops = flops + r1 * self.model.module.forward_flops(
                    (3, 510, 350), alpha=False, beta=True, ratio=False
                )
            ...
            flops = flops + r2 * self.model.module.forward_flops(
                (3, 510, 350), alpha=False, beta=False, ratio=True
            )
