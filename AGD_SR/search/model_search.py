...
class SingleOp(nn.Module):
    ...
    def forward(self, x, beta, ratio):
        ...
        if self.quantize == "search":
            result = (
                (
                    beta[0] * self._op(x, quantize=False)
                    + beta[1] * self._op(x, quantize=True)
                )
                * r_score0
                * r_score1
            )
    ...
    def forward(self, x, alpha, beta, ratio):
        ...
        for w, op in zip(alpha, self._ops):
            if self.quantize == "search":
                result = (
                    result
                    + (beta[0] * op(x, quantize=False) + beta[1] * op(x, quantize=True))
                    * w
                    * r_score0
                    * r_score1
                )
    ...
    def forward_flops(self, size, alpha, beta, ratio):
        ...
        for i, op in enumerate(self.ops):
            if i == 0:
                ...
                flops, size = op.forward_flops(
                    size, alpha[i], beta[i], [ratio[i - 1], 1]
                )
                flops_total.append(flops)
            else:
                flops, size = op.forward_flops(
                    size, alpha[i], beta[i], [ratio[i - 1], ratio[i]]
                )
                flops_total.append(flops)
...
class NAS_GAN(nn.Module):
        ...
        if mode == "arch_ratio":
            ...
            for cell_id in range(self.num_cell):
                ...
                for op_id in range(self.op_per_cell - 1):
                    ratio_cell.append(
                        gumbel_softmax(
                            F.log_softmax(ratio[cell_id][op_id], dim=-1), hard=True
                        )
                    )
    ...
    def forward_flops(self, size, alpha=True, beta=True, ratio=True):
        if alpha:
            ...
            alpha = (
                torch.ones_like(getattr(self, "alpha")).cuda() * 1.0 / len(PRIMITIVES)
            )

        if beta:
            beta = F.softmax(getattr(self, "beta"), dim=-1)
        else:
            beta = torch.ones_like(getattr(self, "beta")).cuda() * 1.0 / 2
    ...
    def _criterion(self, y_hat, x):
        ...
        content_loss = self.content_weight * self.loss_func(
            y_c_features, y_hat_features
        )
    ...
    def _build_arch_parameters(self):
        ...
        setattr(
            ...
            nn.Parameter(
                Variable(
                    1e-3 * torch.ones(self.num_cell, self.op_per_cell, num_ops).cuda(),
                    requires_grad=True,
                )
            ),
        )
        setattr(
            ...
            nn.Parameter(
                Variable(
                    1e-3 * torch.ones(self.num_cell, self.op_per_cell, 2).cuda(),
                    requires_grad=True,
                )
            ),
        )
        ...
        setattr(
            ...
            nn.Parameter(
                Variable(
                    1e-3
                    * torch.ones(
                        self.num_cell, self.op_per_cell - 1, num_widths
                    ).cuda(),
                    requires_grad=True,
                )
            ),
    ...
    def _reset_arch_parameters(self):
        ...
        getattr(self, "alpha").data = Variable(
            1e-3 * torch.ones(self.num_cell, self.op_per_cell, num_ops).cuda(),
            requires_grad=True,
        )
        getattr(self, "beta").data = Variable(
            1e-3 * torch.ones(self.num_cell, self.op_per_cell, 2).cuda(),
            requires_grad=True,
        )
        getattr(self, "ratio").data = Variable(
            1e-3 * torch.ones(self.num_cell, self.op_per_cell - 1, num_widths).cuda(),
            requires_grad=True,
        )
