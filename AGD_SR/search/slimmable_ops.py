...
class USConv2d(QConv2d):
    ...
    def forward(self, input, quantize=False):
        assert self.ratio[0] in self.width_mult_list, (
            str(self.ratio[0]) + " in? " + str(self.width_mult_list)
        )
        self.in_channels = make_divisible(self.in_channels_max * self.ratio[0])
        assert self.ratio[1] in self.width_mult_list, (
            str(self.ratio[1]) + " in? " + str(self.width_mult_list)
        )
        ...
        if quantize:
            ...
            if self.bias is not None:
                qbias = Quantize(
                    bias,
                    num_bits=self.num_bits_weight + self.num_bits,
                    flatten_dims=(0, -1),
                )
            else:
                qbias = None
    ...
    def __init__(self, num_features, width_mult_list=[1.0]):
        ...
        self.bn = nn.ModuleList(
            [
                nn.BatchNorm2d(i)
                for i in [
                    make_divisible(self.num_features_max * width_mult)
